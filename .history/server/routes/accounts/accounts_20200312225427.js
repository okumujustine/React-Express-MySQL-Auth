...
const { doctorUserAuth } = require('./verify_accounts')
//validation imports
        ...
        if(err){
            ...
            if(results.length == 0){  
               ...
               if (!passWordResult){
                   ...
                   const token = jwt.sign({user_id:results[0].user_id, user_role:results[0].user_role}, process.env.LOGIN_TOKEN_SECRET_KEY)
...
router.get('/', doctorUserAuth, (req, res) =>{
    res.send("authorised doctors only")
})

router.get('/', doctorUserAuth, (req, res) =>{
    res.send("authorised anybody logged in")
})
